...
export default async function decorate(block) {
  ...
  // Add Quick Add to Cart UI to the top of checkout__main
  const $main = checkoutFragment.querySelector('.checkout__main');
  if ($main) {
    const quickAddForm = document.createElement('form');
    quickAddForm.id = 'quick-add-to-cart';
    quickAddForm.style.display = 'flex';
    quickAddForm.style.alignItems = 'center';
    quickAddForm.style.gap = '8px';
    quickAddForm.innerHTML = `
      <label>Quick Add to Cart:</label>
      <span>SKU</span>
      <input type="text" id="quick-sku" required style="width:100px;">
      <span>Qty</span>
      <input type="number" id="quick-qty" min="1" value="1" required style="width:60px;">
      <button type="submit" id="quick-add-btn" style="background:#f60;color:#fff;">ADD TO CART</button>
      <div id="quick-add-message" style="margin-left:16px;color:green;"></div>
    `;
    $main.prepend(quickAddForm);

    // Add logic for Quick Add to Cart using dropin cart API
    quickAddForm.addEventListener('submit', async (e) => {
      e.preventDefault();
      const sku = document.getElementById('quick-sku').value.trim();
      const qty = parseInt(document.getElementById('quick-qty').value, 10);
      const messageDiv = document.getElementById('quick-add-message');
      messageDiv.textContent = '';
      if (!sku || qty < 1) {
        messageDiv.textContent = 'Please enter a valid SKU and quantity.';
        return;
      }
      try {
        // Use dropin cart API to add product
        const cart = await cartApi.addProductsToCart([{ sku, quantity: qty }]);
        if (cart) {
          messageDiv.textContent = 'Item added to cart!';
          // Refresh cart UI
          events.emit('cart/data', cart);
        } else {
          messageDiv.textContent = 'Failed to add item. Please check SKU.';
        }
      } catch (err) {
